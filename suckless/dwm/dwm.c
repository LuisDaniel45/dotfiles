...
{
	for (; c && (c->isfloating || !ISVISIBLE(c, selmon) || HIDDEN(c)); c = c->next);
	return c;
}
...
tile(Monitor *m)
{
	...
	if (n > m->nmaster)
		...
		mw = m->ww - m->gap->gappx;
	for (i = 0, my = ty = m->gap->gappx, c = nexttiled(m->cl->clients, m); c; c = nexttiled(c->next, m), i++)
		if (i < m->nmaster) {
